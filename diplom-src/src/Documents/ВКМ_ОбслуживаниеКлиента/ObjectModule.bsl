...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	ОсновнойТекст = СтрШаблон("Специалисту %1 необходимо выполнить работу, а именно: %2 %3 с %4 до %5", Специалист, ОписаниеПроблемы, Формат(ДатаПроведенияРабот, "ДЛФ=Д"), Формат(ВремяНачалаРаботПлан, "ДЛФ=В"), Формат(ВремяОкончанияРаботПлан, "ДЛФ=В"));
			
	Если ЭтоНовый() Тогда
		НовоеУведомление = Справочники.ВКМ_УведомленияТелеграмБоту.СоздатьЭлемент();
		НовоеУведомление.ТекстСообщения = СтрШаблон("Создан новый документ от %1. %2", Формат(Дата, "ДЛФ=Д"), ОсновнойТекст);
	...
	Иначе
		...
		Если ДатаРаботДокумента <> ДатаПроведенияРабот Тогда
			НовоеУведомление = Справочники.ВКМ_УведомленияТелеграмБоту.СоздатьЭлемент();
			НовоеУведомление.ТекстСообщения = СтрШаблон("В документе %1 изменилась плановая дата проведения работ. %2", Ссылка, ОсновнойТекст);
			НовоеУведомление.Записать();
		КонецЕсли;
		
		Если ВремяНачалаДокумента <> ВремяНачалаРаботПлан Тогда
			НовоеУведомление = Справочники.ВКМ_УведомленияТелеграмБоту.СоздатьЭлемент();
			НовоеУведомление.ТекстСообщения = СтрШаблон("В документе %1 изменилось плановое время начала работ. %2", Ссылка, ОсновнойТекст);
			НовоеУведомление.Записать();
		КонецЕсли;
		
		Если ВремяОкончанияДокумента <> ВремяОкончанияРаботПлан Тогда
			НовоеУведомление = Справочники.ВКМ_УведомленияТелеграмБоту.СоздатьЭлемент();
			НовоеУведомление.ТекстСообщения = СтрШаблон("В документе %1 изменилось плановое время окончания работ. %2", Ссылка, ОсновнойТекст);
			НовоеУведомление.Записать();
		КонецЕсли;
		
		Если СпециалистДокумента <> Специалист Тогда
			НовоеУведомление = Справочники.ВКМ_УведомленияТелеграмБоту.СоздатьЭлемент();
			НовоеУведомление.ТекстСообщения = СтрШаблон("В документе %1 изменился специалист. %2", Ссылка, ОсновнойТекст);
			НовоеУведомление.Записать();
		КонецЕсли;
